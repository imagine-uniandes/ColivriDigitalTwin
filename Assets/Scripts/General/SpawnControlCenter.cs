...
[System.Serializable]
public class ControlCameraData
{
    public float x;
    public float y;
    public float z;
...
public class SpawnControlCenter : MonoBehaviour
{
    ...
    private string serverURL = "http://172.24.100.110:8080/data/control-center";
    ...
    IEnumerator UpdateObjectPosition()
    {
        while (true)
        {
            ...
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                ControlCameraData data = JsonUtility.FromJson<ControlCameraData>(json);

                float xPos = data.x;
                float yPos = data.y;
                float zPos = data.z;
                float rotx = data.rotx - 90f;
