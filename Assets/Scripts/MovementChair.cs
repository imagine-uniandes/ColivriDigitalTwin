        ...
        while(true){
            ...
            if (request.result == UnityWebRequest.Result.Success)
            {
                if(b){
                    ...
                    TrackingData data = JsonUtility.FromJson<TrackingData>(json);
                    ...
                    if(Math.Abs(x-prevX) < 0.001 || Math.Abs(y-prevY) < 0.001){
                        ...
                        transform.localPosition = new Vector3(startingPosition.x - y, startingPosition.y, startingPosition.z - x);
